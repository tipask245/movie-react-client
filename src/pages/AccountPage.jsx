...
const AccountPage = () => {
  ...
  const deleteReview = reviewId => {
    setUserInf({
      ...userInf,
      reviews: userInf.reviews.filter(el => el.id !== reviewId)
    })
    movieAPI
      .delete('/movie/delete_review', {
        headers: {
  ...
  const deleteMovieInList = (filmId, listName) => {
    setUserInf({
      ...userInf,
      [listName]: userInf[listName].filter(el => el.id !== filmId)
    })
    movieAPI
      .delete('/movie/remove_from_list', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        },
        data: {
        ...
        <div className="account__body_wrap">
          <div className="account__body">
            <AccountPageTitle title={currentList} />
            {
              ...
              ? (
                  userInf[currentList].reverse().map(el => switchCard(currentList, el))
                ) 
              : (
